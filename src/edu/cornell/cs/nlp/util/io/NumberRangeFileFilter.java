...
class NumberRangeFileFilter implements FileFilter {
	...
	private static int getLastNonNumberIndex(String name, int lastNumberIndex) {
	...
	private static int getLastNumberIndex(String name) {
	...
	public boolean accept(File pathname) {
		...
		final int lastNumberIndex = getLastNumberIndex(name);
		if (lastNumberIndex == -1) {
			return false;
		}
		final int numEndLoc = lastNumberIndex + 1;
		final int numStartLoc = getLastNonNumberIndex(name, lastNumberIndex)
				+ 1;
		final int fileNum = Integer
				.parseInt(name.substring(numStartLoc, numEndLoc));
